...
revenue_1 <- revenue %>% 
             filter(OrgOrder ==1) %>% 
             pivot_longer(revenue_2019:revenue_2021, names_to = "Year", values_to = "Revenue") %>%              mutate(Year = str_remove_all(Year, "revenue_")) 
...
plot_revenues <-
  ...
  layout(
    title = list(text = "Yearly Revenue by Organization"),
...
total_revenue <- baseline_revenue %>%
  ...
  pivot_longer(
    payment_from_sales_initial:fundraising_current,
    names_to = "category",
    values_to = "values"
  )  %>%
  ...
  pivot_wider(c("Organization", "OrgID", "date"),
              names_from = category,
              values_from = values) %>%
  group_by(date) %>%
  mutate(OrgOrder = row_number())
...
all_likert_mode <- baseline_likert_mode %>% 
                   ...
                   pivot_longer(3:9, names_to = "x", values_to = "size")
...
## Likert Chart with bubbles

```{r}

plot_likert_mode <- all_likert_mode %>%   
                    plot_ly(
                      x = ~x,
                      y = ~name,
                      type = "scatter",
  ...
  layout(
    title = list(text = "How confident do you feel about<br>the following aspects of raising capital?",x=.25, xanchor = "left"),
    xaxis = list(title = "",
                  tickvals = c(1,2,3,4,5,6,7),
                  ticktext = c("1<br>Not at all confident", "2", "3", "4", "5","6",  
                              "7<br>Extremely confident"),
                  range = c(0,8)),
    yaxis = list(title = "",
                  tickvals = c(0,1,2,3,4),
                  ticktext = c("Apply to fellowships", "Apply to grant foundations", "Apply to government grants", "Pitch competition", "Pitch investors"),
                 showgrid = FALSE),
                 margin = list(t=75, pad = 5),
                font = plot_font_small,
                colorway = duke_colorway_75[4:5]
  )
...
sample_reduced <- projection_reduced_1 %>% 
                  select(!(initial)) %>% 
                  pivot_longer(current:projected, names_to = "survey") %>% 
                  select(!(Organization))

sample_plot <- sample_reduced %>% 
                plot_ly(
                  y = ~name,
                  x = ~value,
                  name= ~survey,
...
sample_reduced_2 <- projection_reduced_2 %>% 
                  select(!(initial)) %>% 
                  pivot_longer(current:projected, names_to = "survey") %>% 
                  select(!(Organization))

sample_plot_2 <- sample_reduced_2 %>% 
                  plot_ly(
                  y = ~name,
                  x = ~value,
                  name = ~survey,
...
sample_subplot <-  subplot(sample_plot, sample_plot_2,sample_plot, sample_plot_2,sample_plot, sample_plot_2, nrows = 3, shareX = TRUE, margin=.09) %>% 
  layout(title = list(text = "Projected versus Current Funding Sources"),
         barmode = "overlay",
    xaxis = list(title = "Percent", ticksuffix = "%"),
                  colorway = duke_colorway_75[1:2],
                  paper_bgcolor = duke_paperbackground,
                  plot_bgcolor = duke_plotcolor,
                  margin =list(t=50),
                  font = plot_font_extra_small
...
```{r new_projection_plot}


revised_projection_1 <- projection_reduced_1 %>% 
                  select(!(initial)) %>% 
                  pivot_longer(current:projected, names_to = "survey") %>% 
                  select(!(Organization))


revised_projection_2 <- projection_reduced_2 %>% 
                  select(!(initial)) %>% 
                  pivot_longer(current:projected, names_to = "survey") %>% 
                  select(!(Organization))
revised_projection_3 <- projection_reduced_3 %>% 
                  select(!(initial)) %>% 
                  pivot_longer(current:projected, names_to = "survey") %>% 
                  select(!(Organization))
revised_projection_4 <- projection_reduced_4 %>% 
                  select(!(initial)) %>% 
                  pivot_longer(current:projected, names_to = "survey") %>% 
                  select(!(Organization))
revised_projection_5 <- projection_reduced_5 %>% 
                  select(!(initial)) %>% 
                  pivot_longer(current:projected, names_to = "survey") %>% 
                  select(!(Organization))
revised_projection_6 <- projection_reduced_6 %>% 
                  select(!(initial)) %>% 
                  pivot_longer(current:projected, names_to = "survey") %>% 
                  select(!(Organization))

revised_plot_1 <- revised_projection_1 %>% 
                plot_ly(
                  y = ~name,
                  x = ~value,
                  name= ~survey,
...
revised_plot_2 <- revised_projection_2 %>% 
                plot_ly(
                  y = ~name,
                  x = ~value,
                  name= ~survey,
...
revised_plot_3 <- revised_projection_3 %>% 
                plot_ly(
                  y = ~name,
                  x = ~value,
                  name= ~survey,
...
revised_plot_4 <- revised_projection_4 %>% 
                plot_ly(
                  y = ~name,
                  x = ~value,
                  name= ~survey,
...
revised_plot_5 <- revised_projection_5 %>% 
                plot_ly(
                  y = ~name,
                  x = ~value,
                  name= ~survey,
...
revised_plot_6 <- revised_projection_6 %>% 
                plot_ly(
                  y = ~name,
                  x = ~value,
                  name= ~survey,
...
revised_projection_plot <-  subplot(revised_plot_1, revised_plot_2,revised_plot_3,revised_plot_4,revised_plot_5,revised_plot_6, nrows = 3, shareX = TRUE, margin=.09) %>% 
  layout(title = list(text = "Projected versus Current Funding Sources"),
         barmode = "overlay",
    xaxis = list(title = "Percent", ticksuffix = "%"),
                  colorway = duke_colorway_75[1:2],
